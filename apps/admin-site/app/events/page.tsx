...
import { deleteEventItem, listEvents, type EventItem } from "@/lib/admin-api";
  ...
  async function handleDelete(item: EventItem) {
    ...
    try {
      ...
      setEvents((currentEvents) =>
        currentEvents.filter((eventItem) => eventItem.id !== item.id)
      );
    } catch (deleteError) {
      setError(
    ...
    <AdminShell>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Create, update, complete, cancel, or remove event records. Image
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
        ...
        {!isLoading && events.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-background p-6 text-sm text-muted-foreground">
            No events yet. Use Add Event to create the first event record.
          </div>
        ) : null}
