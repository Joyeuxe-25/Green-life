...
import {
  ...
  markDonationMessageRead,
  markDonationMessageUnread,
  type DonationMessageItem
} from "@/lib/admin-api";

export default function DonationMessagesPage() {
  const [messages, setMessages] = useState<DonationMessageItem[]>([]);
      ...
      .catch((loadError) =>
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Could not load donation messages."
        )
      )
      .finally(() => setIsLoading(false));
  }, []);

  async function handleDelete(message: DonationMessageItem) {
    const confirmed = window.confirm(
      `Delete donation message from "${message.donor_name}"?`
    );
    if (!confirmed) {
      return;
    }
    ...
    setError("");
    try {
      await deleteDonationMessage(message.id);
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "Could not delete donation message."
      );
    } finally {
      setBusyId("");
    }
  ...
  async function handleReadToggle(message: DonationMessageItem) {
    setBusyId(message.id);
    setError("");
    try {
      const data =
        message.status === "read"
          ? await markDonationMessageUnread(message.id)
          : await markDonationMessageRead(message.id);
      setMessages((currentMessages) =>
    ...
    <AdminShell>
      <section className="rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Review donation intent messages. Payment processing and email sending
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
        ...
        {isLoading ? (
          <div className="rounded-lg border border-dashed border-border bg-background p-6 text-sm text-muted-foreground">
            Loading donation messages...
          </div>
        ) : null}

        {!isLoading && messages.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-background p-6 text-sm text-muted-foreground">
            No donation messages yet.
          </div>
        ) : null}
        ...
        {!isLoading && messages.length > 0 ? (
          <div className="grid gap-3">
            {messages.map((message) => {
              ...
              const preview = message.message || "No message provided.";
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm font-medium text-foreground">
                        {message.intended_amount || "No intended amount"}
                      </p>
