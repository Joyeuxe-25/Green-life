...
import { deleteNewsItem, listNews, type NewsItem } from "@/lib/admin-api";
...
export default function NewsPage() {
  ...
  async function handleDelete(item: NewsItem) {
    ...
    try {
      ...
      setNews((currentNews) =>
        currentNews.filter((newsItem) => newsItem.id !== item.id)
      );
    } catch (deleteError) {
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "Could not delete news item."
      );
    } finally {
      setDeletingId("");
    }
    ...
    <AdminShell>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Create, update, publish, archive, or remove news items. Image
              upload will come later with R2 media support.
            </p>
          </div>
          <Link
      ...
      <section className="mt-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
        ...
        {!isLoading && news.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-background p-6 text-sm text-muted-foreground">
            No news items yet. Use Add News to create the first draft.
          </div>
        ) : null}
