...
const files = [
  {
    id: "media-green-life-logo-transparent-webp",
...
for (const file of files) {
  ...
  const publicUrl = `${API_BASE_URL}/public/media/file/${file.id}`;
  const mimeType = mimeTypeForPath(file.filePath);
  const stagedPath = path.join(os.tmpdir(), `${file.id}${path.extname(file.filePath).toLowerCase()}`);
...
const sqlPath = path.join(os.tmpdir(), "green-life-rwanda-brand-partner-logos-local.sql");
await writeFile(sqlPath, buildSql(rows), "utf8");

runWrangler(["d1", "execute", DATABASE_NAME, "--local", "--file", sqlPath]);

console.log(`Imported ${rows.length} brand/partner logo files into local R2/D1.`);
...
function runWrangler(args) {
  if (process.platform === "win32") {
    execFileSync(
      "cmd.exe",
      ["/d", "/s", "/c", ["pnpm", "exec", "wrangler", ...args].map(quoteCmdArg).join(" ")],
      {
        cwd: apiDir,
        env: wranglerEnv,
        stdio: "inherit"
...
SET website_url = CASE id
  ...
  ELSE website_url
END,
is_text_only = CASE id
  WHEN 'partner-restore-local' THEN 0
  ELSE is_text_only
END,
updated_at = datetime('now')
WHERE id IN (
